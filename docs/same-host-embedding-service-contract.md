...
5. **TEI local snapshot mode is startup mitigation only.** TEI is started with the cached USER-bge-m3 snapshot directory as `--model-id` so TEI takes its local-model branch instead of the Hugging Face Hub repo path that probes missing ONNX artifacts. It does not add an fd fallback backend; if the snapshot path changes, re-run a controlled startup proof.
...
| TEI startup cache | Compose points TEI `--model-id` at the cached USER-bge-m3 snapshot path under `/data`; keep the volume populated and update the path if the snapshot revision changes |
