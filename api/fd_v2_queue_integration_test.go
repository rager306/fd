...
//nolint:gocritic,unparam // parameter types, batchSize constant, result 1 unused
...
func TestQueueSubmitAndPollCompleted(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		req := httptest.NewRequest(http.MethodGet, "/v1/queue/"+id, http.NoBody) //nolint:gocritic // empty body
		w := httptest.NewRecorder()
		r.ServeHTTP(w, req)
...
func TestQueuePollReturns404ForUnknownId(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/queue/nonexistent", http.NoBody) //nolint:gocritic // empty body
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
