...
func setupQueueTestServer(t *testing.T, queueCap int, batchSize int) (*gin.Engine, *queue.ResultStore, chan queue.Item, *queueTestEmbedder, context.CancelFunc) { //nolint:gocritic,unparam // ignore
	t.Helper()
...
func postQueue(t *testing.T, r http.Handler, body string) *httptest.ResponseRecorder { //nolint:unparam // ignore
...
func TestQueueSubmitAndPollCompleted(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		req := httptest.NewRequest(http.MethodGet, "/v1/queue/"+id, nil) //nolint:gocritic // ignore
		w := httptest.NewRecorder()
		r.ServeHTTP(w, req)
...
func TestQueuePollReturns404ForUnknownId(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/queue/nonexistent", nil) //nolint:gocritic // ignore
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
