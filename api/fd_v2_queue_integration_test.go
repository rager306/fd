...
func setupQueueTestServer(t *testing.T, queueCap int, batchSize int) (*gin.Engine, *queue.ResultStore, chan queue.Item, *queueTestEmbedder, context.CancelFunc) { //nolint:gocritic,unparam // intentional
	t.Helper()
...
func postQueue(t *testing.T, r http.Handler, body string) *httptest.ResponseRecorder { //nolint:unparam // intentional
...
func TestQueueSubmitAndPollCompleted(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		req := httptest.NewRequest(http.MethodGet, "/v1/queue/"+id, nil) //nolint:gocritic // intentional
		w := httptest.NewRecorder()
		r.ServeHTTP(w, req)
...
func TestQueuePollReturns404ForUnknownId(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/queue/nonexistent", nil) //nolint:gocritic // intentional
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
