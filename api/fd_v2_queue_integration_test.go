...
//nolint:unparam // batchSize is required for test configuration
func setupQueueTestServer(t *testing.T, queueCap, batchSize int) (*gin.Engine, *queue.ResultStore, chan queue.Item, *queueTestEmbedder, context.CancelFunc) {
	...
	logger := slog.New(slog.NewTextHandler(io.Discard, nil)) //nolint:gocritic // slog expects nil options
