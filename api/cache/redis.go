...
func (c *RedisCache) key(text string, dim int) string {
	var textBytes []byte
	if text != "" {
		textBytes = unsafe.Slice(unsafe.StringData(text), len(text)) //nolint:gosec // G103: performance optimization for byte casting
	}
	h := sha256.Sum256(textBytes)

	// Pre-allocate enough for the 64 byte hash + ":" + "d" + max int string length
	var buf [100]byte
	hex.Encode(buf[:], h[:])
	...
	return c.prefix + c.namespace + ":" + string(buf[:suffixLen])
}
