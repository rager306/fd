...
func (c *RedisCache) key(text string, dim int) string {
	// Fast path common dimensions to avoid strconv.Itoa allocations.
	if dim == 1024 {
...
func (c *RedisCache) HashText(text string) string {
	...
	// Optimize allocation: encode directly to a stack array instead of hex.EncodeToString heap allocation.
	var dst [64]byte
	hex.Encode(dst[:], h[:])
	return string(dst[:])
}
