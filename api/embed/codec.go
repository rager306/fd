...
func Float32SliceToBytes(slice []float32) []byte {
	...
	if isLittleEndian {
		...
		copy(b, unsafe.Slice((*byte)(unsafe.Pointer(&slice[0])), len(slice)*4)) //nolint:gosec // G103: performance optimization for byte casting
		return b
	}
	b := make([]byte, len(slice)*4)
	for i, v := range slice {
...
func BytesToFloat32Slice(b []byte) []float32 {
	...
	if isLittleEndian {
		...
		copy(unsafe.Slice((*byte)(unsafe.Pointer(&out[0])), len(b)), b) //nolint:gosec // G103: performance optimization for byte casting
		return out
	}
	out := make([]float32, len(b)/4)
	for i := range out {
