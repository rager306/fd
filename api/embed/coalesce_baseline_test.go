...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // G304: Test fixture
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) {
	t.Helper()
	calls = 0 //nolint:ineffassign // test logic
	durations = nil
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // G404: Deterministic seed for tests
