...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // intentional
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls int, totalTexts int, durations []time.Duration) { //nolint:gocritic,unparam // intentional
	t.Helper()
	calls = 0 //nolint:ineffassign // intentional
	durations = nil
	totalTexts = 0 //nolint:ineffassign // intentional
	var mu sync.Mutex
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // intentional
