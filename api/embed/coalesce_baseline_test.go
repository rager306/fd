...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // intentional for tests
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) { //nolint:unparam // testing artifact
	t.Helper()
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // intentional for tests
