...
//nolint:unparam // acceptable in test helper
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) {
	t.Helper()
	calls = 0 //nolint:ineffassign // initialization
	durations = nil
	totalTexts = 0 //nolint:ineffassign // initialization
	var mu sync.Mutex
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // G404: deterministic tests
