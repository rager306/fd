...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // safe file inclusion in test baseline generator
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) { //nolint:unparam // exported return signature is needed later
	t.Helper()
	calls = 0 //nolint:ineffassign // explicit reset for clarity
	durations = nil
	totalTexts = 0 //nolint:ineffassign // explicit reset for clarity
	var mu sync.Mutex
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // non-cryptographic pseudo-randomness for deterministic test baseline generation
