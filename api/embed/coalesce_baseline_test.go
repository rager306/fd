...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // file inclusion is safe in test
	if err != nil {
...
//nolint:unparam // test helper function returns unused metric for completeness
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) {
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // weak rng is fine for test
