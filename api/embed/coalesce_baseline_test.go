...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // G304: test file inclusion
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) { //nolint:unparam // benchmark
	t.Helper()
	calls = 0 //nolint:ineffassign // test
	durations = nil
	totalTexts = 0 //nolint:ineffassign // test
	var mu sync.Mutex
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // G404: test data
