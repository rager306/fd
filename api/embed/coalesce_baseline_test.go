...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // test code
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) { //nolint:unparam // benchmark stub
	t.Helper()
	calls = 0 //nolint:ineffassign // intentional reset in benchmark
	durations = nil
	totalTexts = 0 //nolint:ineffassign // intentional reset in benchmark
	var mu sync.Mutex
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // non-crypto rng in test
