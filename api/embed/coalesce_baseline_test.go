...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // safe in test
	if err != nil {
...
//nolint:unparam,gocritic // result calls is never used, prefer explicitness
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls int, totalTexts int, durations []time.Duration) {
	t.Helper()
	calls = 0 //nolint:ineffassign // intentionally reset
	durations = nil
	totalTexts = 0 //nolint:ineffassign // intentionally reset
	var mu sync.Mutex
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // non-cryptographic use in test
