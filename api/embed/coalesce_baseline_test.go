...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // intentional file inclusion in tests
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) { //nolint:unparam // calls is part of the returned results intended for benchmarking
	t.Helper()
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // intentional weak rng for tests
