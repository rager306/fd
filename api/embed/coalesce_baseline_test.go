...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // G304: test file
	if err != nil {
...
//nolint:unparam // test rig helper
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) {
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // G404: weak random for testing is ok
