...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // Tests read from known testdata
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) {
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // Pseudo-randomness used for deterministic testing
