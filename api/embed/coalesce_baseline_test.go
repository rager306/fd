...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // read is safe
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (int, int, []time.Duration) {
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // safe usage in test
	...
	if len(durations) == 0 {
		return calls, totalTexts, durations
	}
