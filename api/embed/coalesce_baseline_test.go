...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) { //nolint:unparam // result calls is accessed via tests implicitly
	t.Helper()
	var mu sync.Mutex
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // G404: math/rand used for non-cryptographic deterministic test shuffling
