...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // read file in test
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) { //nolint:unparam // benchmark harness signature
	t.Helper()
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // non-crypto rand is fine in tests
