...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // testing file access is safe
	if err != nil {
...
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) { //nolint:unparam // testing param
	t.Helper()
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // non-crypto usage
