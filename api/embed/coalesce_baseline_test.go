...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // expected test file inclusion
	if err != nil {
...
//nolint:unparam // keep return values for API consistency
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls, totalTexts int, durations []time.Duration) {
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // math/rand used for non-cryptographic benchmarking
