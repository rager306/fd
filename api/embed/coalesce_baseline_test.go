...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // safe path in tests
	if err != nil {
...
//nolint:gocritic,unparam // parameter types, calls result unused
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls int, totalTexts int, durations []time.Duration) {
	t.Helper()
	calls = 0 //nolint:ineffassign // reset
	durations = nil
	totalTexts = 0 //nolint:ineffassign // reset
	var mu sync.Mutex
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // weak rng is safe in tests
