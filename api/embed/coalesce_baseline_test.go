...
func load44FZCorpus(t *testing.T) []string {
	...
	data, err := os.ReadFile(root) //nolint:gosec // file comes from relative path in test directory
	if err != nil {
...
//nolint:unparam // benchmark helper
func runCorpusBurst(t *testing.T, e Embedder, texts []string, concurrency int) (calls int, totalTexts int, durations []time.Duration) { //nolint:gocritic // benchmark helper
	t.Helper()
	durations = nil
	var mu sync.Mutex
	...
	rng := rand.New(rand.NewSource(42)) //nolint:gosec // reproducible shuffle in tests
