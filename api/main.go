...
func main() { //nolint:gocyclo // main initialization logic is inherently complex
	...
	if err := lifecycle.AwaitSignalAndShutdown(
		...
		closeResource("local cache", localCache, logger)
		recoveryCancel()
		os.Exit(1) //nolint:gocritic // exitAfterDefer: manually called recoveryCancel before exit
	}
	closeResource("redis", redisCache, logger)
	closeResource("local cache", localCache, logger)
}
