...
func APIKeyAuth(apiKey string) gin.HandlerFunc {
	// Hash the API key once during initialization to avoid re-computing it on every request
	// and to allow secure constant-time comparison of hashes regardless of input length.
	...
	return func(c *gin.Context) {
		...
		// Hash the incoming token to ensure fixed-length comparison
		// mitigating timing attacks that leak the length of the secret.
