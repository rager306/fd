...
func APIKeyAuth(apiKey string) gin.HandlerFunc {
	// 🛡️ Sentinel: Hash the expected API key once during initialization.
	// This prevents length-based timing attacks during comparison and avoids
	// re-hashing the expected key on every request.
	...
	return func(c *gin.Context) {
		...
		// 🛡️ Sentinel: Hash the incoming token before comparison.
		// Comparing hashes ensures both slices are of equal length (32 bytes),
		// preventing subtle.ConstantTimeCompare from returning early and leaking
		// the length of the secret API key.
