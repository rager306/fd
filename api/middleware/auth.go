...
func APIKeyAuth(apiKey string) gin.HandlerFunc {
	// Hash the static expected secret once during middleware initialization
	// to ensure fixed-length slice and prevent length-based timing leaks.
	...
	return func(c *gin.Context) {
		...
		// Hash incoming token so we compare 32 bytes to 32 bytes using ConstantTimeCompare
		// mitigating timing attacks that reveal secret key length.
