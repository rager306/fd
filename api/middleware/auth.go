...
func APIKeyAuth(apiKey string) gin.HandlerFunc {
	// Hash the expected API key once during middleware initialization
	// to avoid re-computing it on every incoming request.
	...
	return func(c *gin.Context) {
		...
		// Hash the provided token before comparison to prevent timing attacks
		// that leak the expected key length, since ConstantTimeCompare returns
		// early if lengths mismatch.
