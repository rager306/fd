...
func APIKeyAuth(apiKey string) gin.HandlerFunc {
	expectedAPIKeyHash := sha256.Sum256([]byte(apiKey))
	return func(c *gin.Context) {
		...
		// Hash both expected and provided token to prevent length-based timing attacks
		if subtle.ConstantTimeCompare(tokenHash[:], expectedAPIKeyHash[:]) != 1 {
