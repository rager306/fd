...
func APIKeyAuth(apiKey string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if subtle.ConstantTimeCompare(tokenHash[:], expectedKeyHash[:]) != 1 {
			// Security: Hashing the tokens before comparison ensures both inputs to ConstantTimeCompare
			// are strictly the same length, preventing timing attacks that could leak the length of the secret API Key.
