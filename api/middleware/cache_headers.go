...
func responseETag(body []byte) string {
	...
	// Optimization: assemble the quoted ETag string directly into a stack-allocated buffer
	// to avoid multiple heap allocations from hex.EncodeToString and string concatenation.
	// Reduces allocations from 3 to 1 per invocation.
	var dst [66]byte
	dst[0] = '"'
