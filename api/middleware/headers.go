...
func newRequestID() string {
	...
	if _, err := rand.Read(bytes[:]); err != nil {
		// ⚡ Bolt optimization: manual zero-allocation hex padding avoids fmt.Sprintf and heap allocations.
		t := time.Now().UnixNano()
		var buf [36]byte
		copy(buf[:24], "00000000-0000-4000-8000-")
		for i := 11; i >= 0; i-- {
			buf[24+i] = "0123456789abcdef"[t&0xf]
			t >>= 4
		}
		return string(buf[:])
	}
	...
	// ⚡ Bolt optimization: single 36-byte stack allocation avoids multiple hex.EncodeToString calls and string concatenations.
	var buf [36]byte
