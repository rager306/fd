...
<tasks>
- [x] **T01**: Added OpenAI-compatible `user` and `priority` request fields, priority validation, and tests for base64/user/priority behavior. _(3h)_
...
- [x] **T02**: Added optional FD_API_KEY bearer auth and CORS/preflight middleware with tests and main wiring. _(3h)_
...
- [x] **T03**: Added opt-in per-IP and per-user rate limiting with X-RateLimit headers and 429 retry envelopes. _(3h)_
...
- [x] **T04**: Added `/v1/batch` endpoint for multiple inner batches with validation, cache/model execution, and tests. _(3h)_
...
- [x] **T05**: Added ETag and Cache-Control middleware for `/v1/embeddings` and `/info` with If-None-Match 304 support. _(2h)_
...
- [x] **T06**: Added `/v1/traces` in-memory request trace ring buffer and endpoint. _(2h)_
...
- [x] **T07**: Added programmatic OpenAPI 3.1 JSON schema and Swagger UI docs endpoints. _(4h)_
...
- [x] **T08**: Added and ran the final M041 45-check fd v2 contract acceptance suite, passing 45/45 against current running fd. _(3h)_
